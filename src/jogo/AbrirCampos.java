...
public class AbrirCampos implements Jogadas {

	private JanelaJogo window;

	public AbrirCampos(int i, int j, JanelaJogo window) {
		this.window = window;
		jogar(i, j, window);
	}

	// Ação de quando botão é clicado em um ponto
	@Override
	public void jogar(int i, int j, JanelaJogo window) {
		...
		if (window.getCampo().getCampoReferencia()[i][j] == 9) {
			...
			new CampoAberto(i,j, window);
		}
		verificarVitoria();
	}

	// Abri o campo quando clidado em um ponto com 0 minas

...
public class JanelaJogo extends JFrame {
	...
	public JanelaJogo(int nMinas, int x, int y) {
		this.campo = new Mapa(nMinas, x, y);
		btn = new Botao[campo.getCampoJogador().length][campo.getCampoJogador()[0].length];

		setSize((campo.getCampoJogador().length) * 42 + 25, (campo.getCampoJogador()[0].length) * 42 + 110);
		...
		for (int i = 0; i < campo.getCampoJogador().length; i++) {
			for (int j = 0; j < campo.getCampoJogador()[0].length; j++) {
				if (campo.getCampoJogador()[i][j] == 0) {
					btn[i][j].setText("");
					btn[i][j].setBackground(new Color(207, 207, 207));
					...
					btn[i][j].setText("");
					btn[i][j].setBackground(new Color(0, 0, 0));
